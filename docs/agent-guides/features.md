...
| PR Pulse | Repo-wide PR badge + popover: checks, Greptile scores, merge conflicts, whose-court ball state, opt-in auto-resume on mechanical bounces | `BoardFeature+PRPulse.swift` (reducer handlers + fetch machinery), `PRPulseButton.swift`, `Supacool/Domain/PRPulse.swift`, `PRBallState.swift`, `Supacool/Clients/PRMonitorClient.swift`, `SupacoolGithubPRClient.swift` | — |
