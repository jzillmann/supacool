...
- **Create a session → spawn a PTY**: `NewTerminalFeature` → `TerminalClient.send(.createTabWithInput(...))` → `WorktreeTerminalManager.handleTabCommand` → `WorktreeTerminalState.createTab(initialInput:)` → ghostty. Ghostty types `initial_input` into the pty at surface spawn, *before* the shell reaches raw mode — and macOS silently truncates a canonical-mode line at 1024 bytes (`MAX_CANON`), newline included. `createTab` therefore wraps any composed launch line over `maxDirectPtyInputBytes` (900) in a temp runner file (`makeWrappedInputLaunch`) and types only the short quoted runner path; the wrapper dir is tracked in `launchScriptDirectories` and removed on tab close. The transcript still records the logical command (`TabCreation.transcriptInput`), not the runner path.
