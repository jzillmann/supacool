...
struct BoardRootView: View {
  ...
  /// Status derivation lives in `BoardSessionClassifier` (shared with the
  /// remote-control MCP tools so remote and on-screen status can't drift).
  private var sessionClassifier: BoardSessionClassifier {
    BoardSessionClassifier(
      terminalManager: terminalManager,
      prReferenceSnapshots: store.state.prReferenceSnapshots,
      reinitializingSessionIDs: store.reinitializingSessionIDs,
      repositories: repositories,
      worktreeInfoByID: worktreeInfoByID
    )
  }
  ...
  private func classify(_ session: AgentSession) -> BoardSessionStatus {
    sessionClassifier.classify(session)
  }

  private func matchedPullRequest(for session: AgentSession) -> GithubPullRequest? {
    sessionClassifier.matchedPullRequest(for: session)
  }
