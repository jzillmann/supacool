  ...
  var body: some Reducer<State, Action> {
    ...
    Reduce { state, action in
      ...
      case ._prRefreshStarted(let refKey):
        return reducePRRefreshStarted(state: &state, refKey: refKey)

      case ._prStateFanout(let refKey, let snapshot):
        return reducePRStateFanout(state: &state, refKey: refKey, snapshot: snapshot)

      // MARK: - PR Pulse (repo-wide open-PR monitoring) — handlers live in BoardFeature+PRPulse.swift

      case .prPulseRepositoriesChanged(let targets):
        return reducePRPulseRepositoriesChanged(state: &state, targets: targets)

      case .prPulseRefreshRequested:
        return reducePRPulseRefreshRequested(state: &state)

      case ._runPRPulseTick:
        return reduceRunPRPulseTick(state: &state)

      case ._prPulseFetchStarted(let repositoryID):
        return reducePRPulseFetchStarted(state: &state, repositoryID: repositoryID)

      case ._prPulseSnapshotLoaded(let snapshot):
        return reducePRPulseSnapshotLoaded(state: &state, snapshot: snapshot)

      case ._prPulseFetchFailed(let repositoryID):
        return reducePRPulseFetchFailed(state: &state, repositoryID: repositoryID)

      case .prPulseIgnoreToggled(let repositoryID, let number):
        return reducePRPulseIgnoreToggled(state: &state, repositoryID: repositoryID, number: number)

      case .prPulseSessionRequested(let repositoryID, let number, let repositories):
        return reducePRPulseSessionRequested(
          state: &state,
...
/// Internal (not private): also thrown by the PR Pulse timeout arms in
/// `BoardFeature+PRPulse.swift`.
nonisolated struct PRRefreshTimeoutError: Error {}
