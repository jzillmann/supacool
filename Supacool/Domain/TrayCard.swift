...
nonisolated enum TrayCardKind: Equatable, Sendable {
  ...
  /// and couldn't be recreated on restore (its branch is gone too). Without
  /// this card the only feedback was a log line: the card just sat there
  /// detached, or — worse, before the worktree rebuild landed — the agent was
  /// resumed in whatever directory the shell fell back to and reported "No
  /// conversation found with session ID". Primary tap focuses the session so
  /// the user can Rerun it.
