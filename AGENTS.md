...
- **File data** (`settings.json`, sessions, bookmarks) lives at a fixed `~/.supacool/` path, *not* under a bundle-id container. The scripts pass the `-SupacoolDataDirectory ~/.supacool-preview-sandbox/.supacool` launch argument to isolate it (delete the sandbox dir to reset the preview). A `$HOME` redirect alone does NOT work — Foundation's `homeDirectoryForCurrentUser` ignores the env var, which used to point "isolated" previews at the real `~/.supacool` until the single-instance guard blocked them. `$HOME` is still redirected as well, for `~/Library` odds and ends and processes spawned inside preview terminals.
